...
#名称
softwareName="valkey-9.1.0"
#软件后缀名
softwareSuffix=".tar.gz"

#下载网址
downloadUrl="https://github.com/valkey-io/valkey/archive/refs/tags/9.1.0.tar.gz"
#安装路径
...
cp ./valkey.conf /usr/local/redis/etc/redis.conf
...
#设置配置文件
if [ ! -d /usr/local/redis/var/run ]; then
	mkdir -m 0777 -p /usr/local/redis/var/run
