...
#名称
softwareName="gettext-0.23"
#软件后缀名
softwareSuffix=".tar.gz"

#下载网址
downloadUrl="http://ftp.gnu.org/pub/gnu/gettext/gettext-0.23.tar.gz"
#安装路径
targetPath="/usr/local"

#校验文件
verifiedFilePath="/usr/local/bin/gettext"
...
if [ -s /usr/bin/gettext ]; then
	rm -rf /usr/bin/gettext
fi
ln -s $targetPath/bin/gettext /usr/bin/gettext
