...
#名称
softwareName="pcre-8.45"
#软件后缀名
softwareSuffix=".tar.bz2"

#下载网址
downloadUrl="https://webwerks.dl.sourceforge.net/project/pcre/pcre/8.45/pcre-8.45.tar.bz2"
#安装路径
targetPath="/usr/local"

#校验文件
verifiedFilePath="/usr/local/bin/pcregrep"
...
if [ -s /usr/lib64/libpcre.so ]; then
	rm -rf /usr/lib64/libpcre.so
fi
ln -s /usr/local/lib/libpcre.so /usr/lib64/libpcre.so
